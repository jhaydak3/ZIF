...
def createList():
	...
	#is it a prime carbon?
		...
		if minDist > 1.8: #then it is a primary carbon
			C2_1ndx = minNdx
			...
			thisPt = carbonCords[C2_1ndx,:] - origin
			thisC = np.dot(invT,thisPt)
			...
			checkifIn = checkifIn and np.all(thisC < 1-pad_factor) and np.all(thisC > pad_factor)
			if checkifIn:
				output_fh.write(str(carbonList[i]) + ' ' + str(carbonList[C2_1ndx]) + ' ' + str(carbonList[C2_2ndx]) + '\n')
	output_fh.close()
...
def main():
	#INPUTS
	global xyz_file
	xyz_file = '/nv/pj1/jhaydak3/FORCE_FIELD_MAKER/lmp_files/neb_empty_300/neb_empty_min.xyz'
	global after_min_data_file 
	after_min_data_file = '/nv/pj1/jhaydak3/FORCE_FIELD_MAKER/lmp_files/neb_empty_300/AFTER_MIN_ROUTINE.data'
	global pad_factor
	pad_factor = .175
